...
*Goal: Setup TailwindCSS, Vite and Icon structure.*
...
*Goal: Establish the Data Model for RBAC.*
...
*Goal: Login screen and responsive dashboard wrapper.*
*Independent Test: Log in using valid credentials and be redirected to a responsive Dashboard with a Sidebar.*
...
*Goal: Restrict actions based on the User's Profile Permissions.*
*Independent Test: A user without permission 'users:read' does not see the 'Usuários' menu link and cannot access `/users`.*
...
*Goal: Standardized CRUD interfaces for Users and Profiles.*
*Independent Test: Admin can list profiles, create a new one, assign permissions, and see icons instead of text for actions.*
...
*Goal: Ensure UI consistency and test coverage.*
