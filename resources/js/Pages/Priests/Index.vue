...
watch(searchInput, (value) => {
  ...
  debounceTimer = setTimeout(() => {
    if (value.length >= 2) {
      router.get('/priests', { search: value }, { preserveState: true })
    } else if (value.length === 0) {
      router.get('/priests', {}, { preserveState: true })
    }
  }, 1000)
})
...
const changePage = (page: number) => {
  router.get('/priests', { page, search: props.search }, { preserveState: true })
}

const confirmDelete = (id: number) => {
  confirm.require({
    message: 'Tem certeza que deseja excluir este padre?',
    header: 'Confirmação',
    icon: 'pi pi-exclamation-triangle',
    acceptLabel: 'Sim',
    rejectLabel: 'Não',
    rejectProps: { severity: 'danger' },
    accept: () => router.delete(`/priests/${id}`),
  })
}
</script>
...
<template>
  <div class="flex flex-col h-full">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-2xl font-bold text-color">Padres</h2>
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <Button
        v-if="can.priestsCreate"
        label="Novo Padre"
        @click="router.get('/priests/create')"
      />
    </div>
    ...
    >
      ...
      >
        ...
        <Column header="Ações" :exportable="false" style="min-width: 8rem">
          <template #body="{ data }">
            ...
            <Button
              ...
              rounded
              severity="warn"
              @click="router.get(`/priests/${data.id}/edit`)"
            />
            <Button
              v-if="can.priestsDelete"
              v-tooltip="'Excluir'"
              icon="pi pi-trash"
              text
              rounded
        ...
        <template #empty>
          <p class="text-center text-muted-color py-4">Nenhum padre encontrado</p>
        </template>
      </DataTable>
