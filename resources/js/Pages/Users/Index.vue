  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex items-center justify-between mb-4">
      <span class="relative w-72">
        ...
        <InputText v-model="searchInput" placeholder="Buscar por nome ou email..." class="w-full pl-10 !bg-white !rounded-xl !shadow-[0_2px_4px_rgba(0,0,0,0.1)]" />
      </span>
      <Button v-if="can.usersCreate" label="Novo Usuário" icon="pi pi-plus" @click="$inertia.get('/users/create')" />
    </div>
    ...
    <div class="bg-surface-ground border border-surface rounded-lg p-4 flex flex-1 flex-col min-h-0">
      ...
      >
        ...
        <Column header="Ações" :exportable="false" style="min-width: 8rem">
          <template #body="{ data }">
            <Button v-if="can.usersRead" v-tooltip="'Visualizar'" icon="pi pi-eye" text rounded severity="info" @click="$inertia.get(`/users/${data.id}`)" />
            <Button v-if="can.usersUpdate" v-tooltip="'Editar'" icon="pi pi-pencil" text rounded severity="warn" @click="$inertia.get(`/users/${data.id}/edit`)" />
            <Button v-if="can.usersDelete" v-tooltip="'Excluir'" icon="pi pi-trash" text rounded severity="danger" @click="confirmDelete(data.id)" />
          </template>
        </Column>
        <template #empty>
