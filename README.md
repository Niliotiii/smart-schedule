...
Projeto base estruturado em AdonisJS e orquestrado por Docker, atuando como um monolito. A infraestrutura visa providenciar rapidamente um ambiente limpo para escalar funcionalidades (gestão de disponibilidades e capacidades, inicialmente para Acólitos, escalável a outras pastorais).
...
2. Execute o comando de inicialização com docker compose na raiz:
   ...
   ```
3. A aplicação estará disponível em `http://localhost:3333`. (Há um endpoint de health check em `http://localhost:3333/health`)
